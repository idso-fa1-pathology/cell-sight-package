...
def run_inference(file_paths, weights_path='backend/models/weights/best.pt'):
    ...
    save_dir = os.path.join('backend', 'uploads', 'runs')
    os.makedirs(save_dir, exist_ok=True)

    for file_path in file_paths:
        prediction_results = model.predict(source=file_path, save=True, imgsz=640, conf=0.5, show_labels=False, save_txt=True, project=save_dir, name='')
